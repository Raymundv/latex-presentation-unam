...
\only<3>{
  \begin{itemize}
    ...
    \ \fbox{\begin{minipage}{.9\textwidth} 
        ...
        \item 1986: Invención de las pinzas ópticas y ejecuci\'on lum\'inica de bacterias.
      \end{minipage}}
    \item 2018: Premio Nobel de Física para Arthur Ashkin por su trabajo con las pinzas ópticas.
  \end{itemize}
  }
...
\only<2>{
  ...
  \caption{\textbf{Figura 1:} Se han encontrado diatomeas en lagos \textbf{(a)} y mares \textbf{(b)}, en cuerpos de agua efímeros como tierra húmeda \textbf{(c)} e incluso en núcleos de hielo en la Antártica \textbf{(d)}.}
    ...
    \end{figure}
\blfootnote{Fuente de la figura \textbf{(d)}: Kelly (2019)}  
  }
...
\blfootnote{Fuentes de las figuras:
    \textbf{(a)} Pedreira, 2012, \textbf{(b-c)} Diatoms of North America, 2023, \textbf{(d)} Microscopy anatomy Olympus, 2022}
}
...
\begin{figure}
	...
	\captionsetup{labelformat=empty}    
    \caption{\textbf{Figura 13}: Preparación de la muestra. Una barrera de grasa de vacío (VG) contiene las suspensiones acuosas con las partículas a ser atrapadas entre el cubreobjetos y el portaobjetos.} %La muestra se coloca en la platina del microscopio para visualizar las partículas y probar la capacidad de atrapamiento de las pinzas ópticas.}% you need long time for this experiments.
    \label{VG}
\end{figure}
...
\begin{figure}
    ...
    \captionsetup{labelformat=empty}
    \caption{\textbf{Figura 15:} Atrapamiento de partículas de $SiO_{2}$ de $24.82 \mu m$. La trampa se produjo con el objetivo de inmersión en agua (MO) y un expansor de haz de 5x (potencia láser establecida en 96 $mW$).}
...
\begin{figure}
     ...
     \end{subfigure}
     \captionsetup{labelformat = empty}
     \caption{\textbf{Figura 17:} Opticution (dead by light) of a yeast cell with a 10x beam expander, and the water immersion microscope objective.}
     \label{opticution}
     \hfill
\end{figure}
...
\end{frame}
\begin{frame}
\bibliographystyle{authoryear} % Or unsrt, abbrv, alpha, apalike, IEEEtran, etc.
